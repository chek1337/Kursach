...
{
	AllocateMemory();
	int node1, node2, node3, node4, node;
	double x1, x2, x3, x4;
	double y1, y2, y3, y4;
	double z1, z2;
	double a0, a1, a2;
	...
	{
		x1 = xy[fe[curFE].node1].x;
		y1 = xy[fe[curFE].node1].y;

		x2 = xy[fe[curFE].node2].x;
		y2 = xy[fe[curFE].node2].y;

		x3 = xy[fe[curFE].node3].x;
		y3 = xy[fe[curFE].node3].y;

		x4 = xy[fe[curFE].node4].x;
		y4 = xy[fe[curFE].node4].y;

		z1 = z[fe[curFE].bottom];
		z2 = z[fe[curFE].top];
		...
		// Короче, поменял обход области. До этого было, что я обхожу сначала все конечные элементы,
		...
		for (int lvl = 0; lvl < NoN_z - 1; lvl++) // Здесь как раз за это и отвечает этот цикл
		{
		...
		int numOfFun = 0; // нужен будет потом, когда будут разные функции на разных на КЭ 
...
double GridAndSLAE::Phi(double e, double n, int i, int j, double b1, double b2, double b3, double b4, double b5, double b6, double a0, double a1, double a2) // Ммммммм список аргументов
{
	...
	switch (i) // Не стал делать для всяки[ "b6*e + b3" отдельные переменные, чтобы легче можно было формулам проверять 
	{
	case 0:
		// phi = (1 - e)*(1 - n)
		// d(phi)/de = -1 + n
